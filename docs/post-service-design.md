...
| MQ | RocketMQ(仅 fanout,topic `youjianxin_dating_post_fanout_v1`);计数刷盘仍走 Redis + `@Scheduled` |
...
- 写扩散经 RocketMQ topic `youjianxin_dating_post_fanout_v1` 解耦,Consumer 同进程内消费(见 §10.2.2)。
...
| MQ | RocketMQ + `rocketmq-spring-boot-starter`(与 Spring Boot 3.3 兼容版本)| **仅 fanout 场景**;topic / group 必带 `youjianxin_dating_` 前缀(CLAUDE.md 隔离前缀) |
...
```
com.dating.post
├── PostApplication                # @SpringBootApplication + @EnableScheduling
├── grpc/
...
│   └── consumer/
│       └── PostFanoutConsumer      # 消费 fanout msg,拉 followers 后 ZADD timeline
├── job/                            # 定时任务,@Scheduled + ShedLock 多实例互斥
│   ├── LikeFlushJob                # 每分钟刷盘点赞
│   ├── CommentFlushJob             # 每分钟刷盘评论
│   └── FeedScoreJob                # 每 5 分钟重建池
├── manager/                        # 单聚合 + 缓存读写
...
**统一返回包装**:proto 里 `BaseResponse{code, message, extra}`。业务异常由 `GlobalExceptionHandler` 兜底转 `BaseResponse(code != 0)`。gRPC 状态码统一 OK,业务错误靠 code 区分。
...
业务异常由 `GlobalExceptionHandler` 兜底:`BizException` → proto `BaseResponse{code, message}`,gRPC 层统一 OK。
...
App ─ CreatePost(content, [image_key]) ─→ post-service
   ...
   6. PostFanoutProducer.syncSend(topic=youjianxin_dating_post_fanout_v1,
        msg={postId, authorUserId, createdAtEpoch})
      - 本地 retry 3 次,timeout 2s/次
      - 3 次全失败 → log.error + post.fanout.produce.fail +1,不阻塞返回
      - Consumer 异步消费做实际写扩散,详见 §10.2.2
   7. 返回 post_id
```

**事务边界只覆盖步骤 3** —— Redis 写、冷启动池写、MQ 发送都是事务外的 best-effort,失败不回滚 DB(帖子已落库,后续 5 分钟池重建会兜底)。
...
| ② 好友时间线 `user:timeline:{user_id}` | 发帖 → RocketMQ → Consumer 写扩散 | 第 3 位强插好友 | 时间倒序 | ❌ (per-user) |
...
#### 10.2.2 池 ② 好友时间线(push 模式 / RocketMQ 写扩散)
...
**为什么用 MQ 而不是 `@Async`**(v0.3 调整,前置评审通过):

| 维度 | `@Async`(原方案) | RocketMQ(现方案) |
|---|---|---|
| 进程崩溃 | 线程池里未跑的任务直接丢 | Broker 持久化,Consumer 重启继续 |
| 关注者多 | 单线程 ZADD N 次,阻塞 `@Async` 线程池 | Consumer 并发拉,多实例分担 |
| 失败重试 | 抛异常即丢 | 自动重投 16 次 → 仍失败转 DLQ |
| 扩容吞吐 | 加 `@Async` 线程池容量(仍单进程) | 加 Consumer 实例,水平扩展 |
| 跟发帖事务关系 | 同 JVM 事务后调度 | 事务 COMMIT 后 syncSend,失败不回滚 DB |

**触发**:`PostWriteService.createPost` 事务 COMMIT 之后,同步调 `PostFanoutProducer.syncSend` 发到 topic `youjianxin_dating_post_fanout_v1`(§9.1 第 6 步)。

**事务-消息一致性**:**不上 Outbox**,接受"事务提交但 MQ 发送失败"这个极小窗口,理由:

- Producer 本地 retry 3 次(timeout 2s/次),正常 Broker 抖动可覆盖
- 3 次都失败 → `log.error` + `post.fanout.produce.fail` Counter +1,不阻塞接口返回
- 5 分钟后全网热门池 ① 重建可让该帖从主力位曝光,用户感知极小
...
```json
{
  "postId":         12345,
  "authorUserId":   999,
  "createdAtEpoch": 1781000000
}
```

不预拉 followers 的原因:

- 关注列表是动态的,Producer 拉时和 Consumer 处理时可能已不同 → 以 Consumer 时点为准更新鲜
- msg 体积小,Broker 落盘 / 网络传输开销低
- 同一作者多帖 fanout 时,Consumer 端的 Caffeine 30s 缓存还能命中
...
public class PostFanoutProducer {
    private static final String TOPIC = "youjianxin_dating_post_fanout_v1";
    private final RocketMQTemplate template;
    private final MeterRegistry metrics;

    public void send(long postId, long authorUserId, long createdAtEpoch) {
        var msg = new FanoutMessage(postId, authorUserId, createdAtEpoch);
        for (int i = 0; i < 3; i++) {
            try {
...
```
┌── Step 1: Producer(发帖事务 COMMIT 后,同步)──────────────────┐
│  rocketMQTemplate.syncSend(topic, {postId, authorUserId, ts})│
│    本地 retry 3 次,2s timeout/次                              │
│    全失败 → log.error + post.fanout.produce.fail +1           │
└──────────────────────────────────────────────────────────────┘
                              │  毫秒级,Broker 持久化
                              ▼
┌── Step 2: Consumer(同进程内,CONCURRENTLY)───────────────────┐
│  followers = userClient.getFriendUserIds(authorUserId)      │
...
└──────────────────────────────────────────────────────────────┘
                              ...
                              ▼
┌── Step 3: 死信(16 次仍失败)─────────────────────────────────┐
│  转发到 %DLQ%youjianxin_dating_post_service_fanout              │
...
ZADD 同 key 同 member 是覆盖语义,score 用 `createdAtEpoch` 是固定值(不是消费时刻),**重投无副作用** —— 无论第几次重试,timeline 里那条记录的排序位置都一致,不会因为"消息延迟到达"而排到错误位置。所以不需要额外幂等表。

**为什么 score 用 epoch 而不是热度分**:

timeline 的产品语义是「按时间看好友最近发啥」,读时直接 `ZREVRANGEBYSCORE` 取最新 N 条。热度排序由全网热门池 ① 负责,timeline 不重复算。

**为什么 timeline 不分性别**:

timeline 是 per-user 私有视图,关注谁是用户自己定的,服务端不再二次分桶。

**为什么 Consumer 用 CONCURRENTLY 而不是 ORDERLY**:

- 同一帖的 fanout 只有一条 msg,自身没有顺序问题
- 不同帖之间,timeline 排序由 ZSet `score = epoch` 决定,跟消费顺序无关
- 并发消费提高吞吐,关注数大的发帖人不阻塞别人

**为什么 Consumer 不用 ShedLock**:

RocketMQ Consumer Group 天然按 queue 分配,多实例自动负载均衡且不会重复消费同一条 msg。ShedLock 只用于 `@Scheduled` Job(`LikeFlushJob` / `CommentFlushJob` / `FeedScoreJob`,见 §6.5)。

**失败模式**:

| 场景 | 后果 | 兜底 |
|---|---|---|
| Producer 3 次重试都失败 | 该帖没产生 fanout msg | 5 分钟后全网热门池 ① 重建覆盖,用户能从主力位看到 |
| Broker 挂 | Producer 全失败,既有 msg 在 Broker 盘上等恢复 | 同上,主力池兜底;Broker 恢复后无新积压(失败的 msg 没发出) |
| user-service down | Consumer RPC 异常,自动重投 | 等上游恢复;16 次仍失败进 DLQ,人工补 |
| 关注关系变更 | 老 timeline 不回溯重写 | 新关注 → 从新发帖开始入 timeline;想看旧帖去 user-service 拉 TA 的帖子列表 |
| Consumer 进程崩溃 | 已 ACK 的 msg 不再消费;未 ACK 自动重投到别的实例 | Broker 持久化,Consumer 重启即继续 |
...
**触发**:`PostWriteService.createPost` 流程的第 5 步,事务提交后**同步** ZADD(只是一个 ZADD,毫秒级,没必要走 MQ;走 MQ 反而让"新帖立刻有曝光位"变成"几秒后才有",破坏 §10.2.3 的核心卖点)。
...
| 维度 | ① 全网热门池 | ② 好友时间线 | ③ 冷启动池 |
|---|---|---|---|
| 模式 | pull (Job 主动拉) | push (MQ 异步写扩散到关注者) | push (写到自己性别池) |
| 写触发 | 每 5 分钟 | 发帖 → RocketMQ → Consumer | 发帖同步 |
| 数据规模 | 全网 × 3 天,Top 3000/性别 | per-user × 100 条 | 全网 × 7 天 ≤ 10000/性别 |
| 排序 | Hacker News 热度分 | epoch 倒序 | epoch 倒序 |
| 性别分桶 | ✅ 按发帖人性别 | ❌ per-user 私有 | ✅ 按发帖人性别 |
| 写失败后果 | 池陈旧,读降级 | Producer 失败 → 5 分钟后 ① 兜底;Consumer 失败 → 自动重投 16 次 → DLQ | 该新帖少一路曝光,仍可能进 ① |
...
| 写扩散到 5000 关注者中途崩 | 部分人 timeline 缺这帖 | MQ Consumer 自动重投(最多 16 次)→ 仍失败转 DLQ;5 分钟后全网池重建兜底 |
| MQ 消费重试导致 fanout 重复执行 | 同 (follower, postId) 多次 ZADD | ZADD 同 score(=createdAtEpoch)同 member 天然幂等,重投无副作用,不需要额外幂等表 |
...
| `post.fanout.produce.fail` | Counter | Producer 重试 3 次仍失败的发帖数(每帖至多 +1)|
| `post.fanout.consume.latency` | Timer | 单条 fanout 消息处理耗时 |
| `post.fanout.dlq.size` | Gauge | DLQ 积压数,>0 立即人工排查 |
...
- [ ] `post-service/pom.xml` 加依赖:
  ...
  - `rocketmq-spring-boot-starter`(fanout 用,选与 Spring Boot 3.3 兼容版本)
...
- [ ] 先在 service 层单元测试跑通增 / 查 / 删 / 点赞 / 评论;集成测试留到 Day 6 用 `grpcurl` 测

### Day 6:gRPC + Feed

- [ ] `PostGrpcService` 实现 9 个 RPC
- [ ] `UserClient`(先用桩,getFriendUserIds 返空、isMale 取模)
- [ ] `mq/producer/PostFanoutProducer`(`syncSend` + 本地 3 次 retry + `post.fanout.produce.fail` 指标)
- [ ] `mq/consumer/PostFanoutConsumer`(`@RocketMQMessageListener` CONCURRENTLY + 拉 followers + ZADD/裁剪/TTL)
- [ ] Nacos `post-service-dev.yaml` 加 RocketMQ name-server 配置(从 `dev-onboarding.md` 取地址)
- [ ] `FeedService.rebuildRecommendPool` + `getRecommendFeed` + `mergeThreeWay`
- [ ] `job/LikeFlushJob` / `job/CommentFlushJob` / `job/FeedScoreJob`(`@EnableScheduling`)
...
| 5 | 引入清单外中间件 | PG + Redis + MinIO + **RocketMQ(经评审,仅 fanout 场景,见 §10.2.2;topic/group 带 `youjianxin_dating_` 前缀)**;其他场景(计数刷盘等)仍走 Redis;ES / Mongo / ZK 等仍禁 |
...
| 9 | 跨服务事务 | `@Transactional` 内不嵌远程调用;写扩散经 MQ 在事务外,Producer 失败不回滚 |
| 10 | controller 直调 mapper | 严格 `grpc → service → manager → mapper` 单向;定时任务在独立 `job/` 包,只调 service / manager |
