...
| MQ | RocketMQ(仅 fanout,topic `dev_youjianxin_post_fanout_v1`);计数刷盘仍走 Redis + `@Scheduled` |
...
- 写扩散经 RocketMQ topic `dev_youjianxin_post_fanout_v1` 解耦,Consumer 同进程内消费(见 §10.2.2)。
...
| MQ | RocketMQ + `rocketmq-spring-boot-starter`(与 Spring Boot 3.3 兼容版本)| **仅 fanout 场景**;topic / group 必带 `dev_youjianxin_` 前缀(CLAUDE.md 隔离前缀) |
...
App ─ CreatePost(content, [image_key]) ─→ post-service
   ...
   6. PostFanoutProducer.syncSend(topic=dev_youjianxin_post_fanout_v1,
...
**触发**:`PostWriteService.createPost` 事务 COMMIT 之后,同步调 `PostFanoutProducer.syncSend` 发到 topic `dev_youjianxin_post_fanout_v1`(§9.1 第 6 步)。
...
public class PostFanoutProducer {
    private static final String TOPIC = "dev_youjianxin_post_fanout_v1";
...
└──────────────────────────────────────────────────────────────┘
                              ...
                              ▼
┌── Step 3: 死信(16 次仍失败)─────────────────────────────────┐
│  转发到 %DLQ%dev_youjianxin_post_service_fanout              │
...
| 5 | 引入清单外中间件 | PG + Redis + MinIO + **RocketMQ(经评审,仅 fanout 场景,见 §10.2.2;topic/group 带 `dev_youjianxin_` 前缀)**;其他场景(计数刷盘等)仍走 Redis;ES / Mongo / ZK 等仍禁 |
